...
**Swagger (khi bật trên môi trường dev):** `{API_BASE}/swagger` — tra cứu path, body, response.  
...
- **Thêm bắt buộc cho trải nghiệm nhóm:**
  ...
  - **Invite join** (`shareCode` + `POST .../join` / `join-guest`), **`guestKey`**, toolkit khẩn cấp **có `journeyId`** + SignalR **`EmergencyPlaceSelected`**, map qua **`GET .../polyline`** sau khi đã join — **không** dùng `GET .../shared/{code}` cho việc này (API đó chỉ cho **chuyến đã complete** — mục 1.5).
...
### 1.5 Hai ý nghĩa của “share” — **tách biệt**, đừng lộn

Cùng một lần **`POST /api/journeys/{journeyId}/share`** (owner) tạo **`shareCode`** / **`shareLink`** dùng để **mời người vào chuyến**; nhưng **từng API public** bên dưới phục vụ **hai nghiệp vụ khác nhau**:

**A) Xem lại chuyến đã đi — “gallery / lịch sử” (không phải join live)**  

| API | Điều kiện backend | Mục đích |
|-----|-------------------|----------|
| **`GET /api/journeys/shared/{shareCode}`** | Chỉ trả **200** khi journey có **`status = completed`** (và `shared_journeys.is_active`). | Trả **`PublicSharedJourneyDetailResponse`**: tuyến, waypoint, feedback công khai… để **người khác xem tổng quan chuyến đã hoàn thành**. |
| **`GET /api/journeys/shared`** (list + phân trang) | Chỉ các chuyến **đã completed**. | Feed “chuyến đã share” kiểu khám phá. |

→ Trong lúc chuyến **chưa complete**, gọi **`GET .../shared/{shareCode}`** sẽ **404** — đúng thiết kế: đây **không** phải API preview cho người sắp join multiplayer.

**B) Vào chuyến cùng nhóm — multiplayer (chuyến đang setup / đang chạy)**  

| API | Ghi chú |
|-----|---------|
| **`POST /api/journeys/shared/{shareCode}/join`** (Bearer) | Thêm member; **không** yêu cầu journey đã complete. |
| **`POST /api/journeys/shared/{shareCode}/join-guest`** | Guest + `guestKey`. |

**Map tổng quan giống owner (đang join / đã join):** dùng **`GET /api/journeys/{journeyId}/polyline`** (JWT hoặc `guestKey`), và khi cần **`GET /api/journeys/{journeyId}`** (nếu app/ policy cho phép). **Sau** `join` client đã có `journeyId` + `guestKey?`.

**Deep link:** `shareLink` mở đúng màn **`join`** (gọi POST join); **không** coi `GET shared/{code}` là bước bắt buộc trước join cho chuyến live.

*Nếu product muốn “xem trước tuyến trước khi bấm join” mà **không** gọi join — cần **endpoint riêng** (hiện backend **chưa** có).*

### 1.6 Chọn **Solo vs Multi** trên app (gợi ý FE)

Backend không có field riêng “đang solo hay multi” trên mọi response; **chuẩn nhất** là dựa vào **`GET .../waypoints/attendance`** → field **`activeMemberCount`**:

- **`activeMemberCount <= 1`** → coi như **Solo** (không hub, không tooltip x/N, không live-location; có thể **không** gọi lại attendance).
- **`activeMemberCount >= 2`** → **Multi** (bật đầy đủ mục 0.2).

Gọi **một lần** khi vào màn hành trình đang chạy / sau khi ai đó join hoặc **leave** (để đổi chế độ khi chỉ còn owner). Đây **không** trái mục 0.1: 0.1 nói **không làm UI tooltip / không poll** cho solo; **không** cấm một request đọc `activeMemberCount` để branch.
...
**Solo: bỏ qua mục này.** Chỉ khi app đang ở chế độ nhóm (đã join ≥2 người active hoặc màn chờ multiplayer):

- Kết nối hub: `wss://.../hubs/journey-live?access_token=<URL_ENCODED_JWT>` (nếu có JWT).
- Guest **không JWT**: vẫn connect hub; **`JoinJourney`** / **`UpdateLocation`** kèm **`guestKey`** khi cần.
- **`JoinJourney` được phép trước khi owner `start`** (phòng chờ) để nhận SignalR **`MemberJoined`**, **`MemberLeft`**, **`JourneyStarted`** (tên method đúng như server). GPS (`UpdateLocation` / `live-location`) vẫn **sau start**.
...
### 4.1 Share lịch sử, invite join, và public feed

| Method | Path | Auth | Ghi chú |
|--------|------|------|---------|
| POST | `/api/journeys/{journeyId}/share` | Bearer | Tạo `shareCode` / `shareLink` (dùng cho **join** multiplayer + sau này link **xem lịch sử** khi đã complete). |
| GET | `/api/journeys/shared/{shareCode}` | Không | **Chỉ chuyến `completed`.** Chi tiết public để **xem lại hành trình đã đi** — **không** dùng làm preview trước join (mục 1.5). |
| GET | `/api/journeys/shared` | Không | List các chuyến share **đã completed** (feed). |
| POST | `/api/journeys/shared/{shareCode}/join` | Bearer | **Multiplayer:** thêm member vào chuyến **đang mở** (chưa complete/cancel). |
| POST | `/api/journeys/shared/{shareCode}/join-guest` | Không | Body: `displayName`, `guestKey?` |

**Roster phòng chờ / danh sách người đang đi**

| Method | Path | Auth | Ghi chú |
|--------|------|------|---------|
| GET | `/api/journeys/{journeyId}/members?guestKey=` | Bearer **hoặc** `guestKey` (giống `polyline`) | Trả mảng thành viên **active**; **không** yêu cầu journey đã `start` — dùng **poll ~5s** trên màn chờ / overview. Sau `start`, `latitude` / `longitude` có thể có nếu đã có cache GPS (giống snapshot `live-locations`). **Owner** được ghi vào roster khi gọi **`POST .../share`** (đừng bỏ qua bước share trước lobby). |

Response từng phần tử (camelCase): `memberId`, `displayName`, `role` (`owner` | `member`), `isGuest`, `joinedAt`, `latitude?`, `longitude?`.
...
|-------|-------------------|----------|
| **`MemberJoined`** | `journeyId`, `memberId`, `displayName`, `role`, `isGuest` | Cập nhật roster phòng chờ (có thể kết hợp poll **`GET .../members`**). |
| **`MemberLeft`** | `journeyId`, `memberId` | Gỡ người khỏi UI lobby. |
| **`JourneyStarted`** | `journeyId`, `startedAt` | Chuyển khỏi màn chờ; bật GPS nhóm / `live-location`. |
...
|--------|---------|---------|
| `JoinJourney` | `journeyId`, `guestKey?` | **Bắt buộc** sau connect để vào group journey. **Cho phép trước start** (lobby). Journey **completed/cancelled** → lỗi hub. |
| `LeaveJourney` | `journeyId` | Rời group. |
| `UpdateLocation` | `journeyId`, `latitude`, `longitude`, `guestKey?`, `accuracyMeters?`, `headingDegrees?` | **Chỉ sau khi journey đã start.** Cùng pipeline Redis + broadcast như REST. **Khuyến nghị:** ưu tiên **`POST live-location`**. |
...
1. Deep link / `shareCode` → màn join → **`POST .../join`** hoặc **`join-guest`** ( **không** gọi `GET .../shared/{code}` cho chuyến live — API đó chỉ cho **đã complete**, sẽ 404).
2. Lưu `journeyId`, `guestKey?`, role từ response join.
3. Ngay sau đó **`GET .../polyline`** (và nếu cần **`GET .../{journeyId}`**) để vẽ map tổng quan.
4. Trên **màn chờ:** mở hub → **`JoinJourney`** (được phép **trước** `start`) → lắng nghe **`MemberJoined`**, **`MemberLeft`**, **`JourneyStarted`**. Giữ poll **`GET .../members`** nếu muốn đồng bộ khi miss event / reconnect.

**Bước B — Màn đang đi (multi)**

1. **`GET .../polyline`** (± `guestKey`, ± GPS).
2. **`GET .../live-locations`**, đảm bảo hub đã **`JoinJourney`**, listener **`MemberLocationUpdated`**.
...
| Hub **`HubException`** | **Multi:** chưa `JoinJourney`; journey đã kết thúc; **GPS / hub UpdateLocation / live-location REST** khi chưa start; GPS invalid |

---

## 9. Tệp backend tham chiếu

- `SharedJourneyRepository.GetPublicDetailAsync` / `GetPublicCompletedAsync` — filter **`completed`**; đừng lộn với `GetByShareCodeWithJourneyAsync` (dùng cho **join**, không filter complete).
- `JourneyController.cs` — polyline, **`GET .../members`**, attendance, live-location, lifecycle, guest.
- `JourneyMemberRosterItemResponse` — payload roster.
- `JourneyShareService.ShareJourneyAsync` — sau share gọi `EnsureOwnerMemberAsync` để owner có trong DB trước `start`.
...
- DTO: `UpdateLiveLocationRequest`, `JourneyMemberLocationNotification`, `JourneyMemberJoinedNotification`, `JourneyMemberLeftNotification`, `JourneyStartedLiveNotification`, `JourneyWaypointAttendanceResponse`, …
